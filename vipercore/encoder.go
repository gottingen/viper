...
func CapitalLevelEncoder(l Level, enc PrimitiveArrayEncoder) {
	enc.WriteString(l.CapitalString())
}
...
func ISO8601TimeEncoder(t time.Time, enc PrimitiveArrayEncoder) {
	enc.WriteString(t.Format("2006-01-02T15:04:05.000Z0700"))
}

// RFC3339TimeEncoder serializes a time.Time to an RFC3339-formatted string.
func RFC3339TimeEncoder(t time.Time, enc PrimitiveArrayEncoder) {
	enc.WriteString(t.Format(time.RFC3339))
}

// RFC3339NanoTimeEncoder serializes a time.Time to an RFC3339-formatted string
// with nanosecond precision.
func RFC3339NanoTimeEncoder(t time.Time, enc PrimitiveArrayEncoder) {
	enc.WriteString(t.Format(time.RFC3339Nano))
}
...
func ShortCallerEncoder(caller EntryCaller, enc PrimitiveArrayEncoder) {
	// TODO: consider using a byte-oriented API to save an allocation.
	enc.WriteString(caller.TrimmedPath())
}
...
type PrimitiveArrayEncoder interface {
	// Built-in types.
	WriteBool(bool)
	WriteByteString([]byte) // for UTF-8 encoded bytes
