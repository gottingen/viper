...
func TestFieldAddingError(t *testing.T) {
	...
	tests := []struct {
		...
		{t: StringerType, iface: obj{}, want: empty, err: "PANIC=interface conversion: vipercore_test.obj is not fmt.Stringer: missing method String"},
...
func TestEquals(t *testing.T) {
	tests := []struct {
		...
		{
			a:    viper.Time("k", time.Unix(1000, 1000)),
			b:    viper.Time("k", time.Unix(1000, 1000)),
			want: true,
		},
		{
			a:    viper.Time("k", time.Unix(1000, 1000).In(time.UTC)),
			b:    viper.Time("k", time.Unix(1000, 1000).In(time.FixedZone("TEST", -8))),
			want: false,
		},
		{
			a:    viper.Time("k", time.Unix(1000, 1000)),
			b:    viper.Time("k", time.Unix(1000, 2000)),
			want: false,
		},
		{
			a:    viper.Binary("k", []byte{1, 2}),
			b:    viper.Binary("k", []byte{1, 2}),
			want: true,
		},
		{
			a:    viper.Binary("k", []byte{1, 2}),
			b:    viper.Binary("k", []byte{1, 3}),
			want: false,
		},
		{
			a:    viper.ByteString("k", []byte("abc")),
			b:    viper.ByteString("k", []byte("abc")),
			want: true,
		},
		{
			a:    viper.ByteString("k", []byte("abc")),
			b:    viper.ByteString("k", []byte("abd")),
			want: false,
		},
		{
			a:    viper.Ints("k", []int{1, 2}),
			b:    viper.Ints("k", []int{1, 2}),
			want: true,
		},
		{
			a:    viper.Ints("k", []int{1, 2}),
			b:    viper.Ints("k", []int{1, 3}),
			want: false,
		},
		{
			a:    viper.Object("k", users(10)),
			b:    viper.Object("k", users(10)),
			want: true,
		},
		{
			a:    viper.Object("k", users(10)),
			b:    viper.Object("k", users(20)),
			want: false,
		},
		{
			a:    viper.Any("k", map[string]string{"a": "b"}),
			b:    viper.Any("k", map[string]string{"a": "b"}),
			want: true,
		},
		{
			a:    viper.Any("k", map[string]string{"a": "b"}),
			b:    viper.Any("k", map[string]string{"a": "d"}),
			want: false,
		},
	}
