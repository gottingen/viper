...
func TestReplaceGlobals(t *testing.T) {
	...
	withLogger(t, DebugLevel, nil, func(l *Logger, logs *observer.ObservedLogs) {
		...
		expected := observer.LoggedEntry{
			Entry:   vipercore.Entry{Message: "captured"},
			Context: []Field{},
		}
		assert.Equal(
			t,
...
func TestGlobalsConcurrentUse(t *testing.T) {
	...
	vtest.Sleep(100 * time.Millisecond)
	stop.Toggle()
	wg.Wait()
}
...
func TestNewStdLogAt(t *testing.T) {
	...
	levels := []vipercore.Level{DebugLevel, InfoLevel, WarnLevel, ErrorLevel, DPanicLevel}
...
func TestNewStdLogAtPanics(t *testing.T) {
	...
	levels := []vipercore.Level{DPanicLevel, PanicLevel}
	for _, level := range levels {
...
func TestNewStdLogAtInvalid(t *testing.T) {
	_, err := NewStdLogAt(NewNop(), vipercore.Level(99))
...
func TestRedirectStdLog(t *testing.T) {
	...
	withLogger(t, DebugLevel, nil, func(l *Logger, logs *observer.ObservedLogs) {
		...
		assert.Equal(t, []observer.LoggedEntry{{
			Entry:   vipercore.Entry{Message: "redirected"},
			Context: []Field{},
...
func TestRedirectStdLogAt(t *testing.T) {
	...
	for _, level := range levels {
		withLogger(t, DebugLevel, nil, func(l *Logger, logs *observer.ObservedLogs) {
			...
			assert.Equal(t, []observer.LoggedEntry{{
				Entry:   vipercore.Entry{Level: level, Message: "redirected"},
				Context: []Field{},
			}}, logs.AllUntimed(), "Unexpected global log output.")
		})
	}
...
func TestRedirectStdLogAtInvalid(t *testing.T) {
	restore, err := RedirectStdLogAt(NewNop(), vipercore.Level(99))
	defer func() {
		if restore != nil {
			restore()
		}
